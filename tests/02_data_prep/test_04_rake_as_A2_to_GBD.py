...
FHS_DATA_PATH = str(mbpc.AGE_SPECIFIC_FHS_PATH)
...
aa_raked_read_paths = {
    'malaria':    mbpc.MAL_RAKED_AA_READ_PATH,
    'malaria_pf': mbpc.MAL_RAKED_AA_READ_PATH,
    'malaria_pv': mbpc.MAL_RAKED_AA_READ_PATH,
    'dengue':     mbpc.DEN_RAKED_AA_READ_PATH,
}

years = list(range(2000, 2023))
year_filter = ('year_id', 'in', years)

as_full_population_df_path = mbpc.POPULATION_READ_PATH / "as_2023_full_population_df.parquet"
if not as_full_population_df_path.exists():
    pytest.skip(
...
full_2023_hierarchy_path = mbpc.HIERARCHY_READ_PATH / f"full_hierarchy_2023_{mbpc.LSAE_HIERARCHY}.parquet"
...
for cause in cause_map:
    ...
    as_full_cause_df_path = TEST_DIR / f"as_full_{cause}_df.parquet"
    as_full_cause_ds_path = TEST_DIR / f"as_full_{cause}_ds.nc"
