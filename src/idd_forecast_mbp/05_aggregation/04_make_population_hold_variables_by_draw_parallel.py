...
wf_uuid = uuid.uuid4()
tool_name = f"{package_name}_make_population_hold_variables_by_draw_{wf_uuid}"
...
task_template = tool.get_task_template(
    template_name="make_population_hold_variables_by_draw",
    ...
    command_template=(
        "python {script_root}/make_population_hold_variables_by_draw.py "
        "--cause {{cause}} "
    ...
    node_args=["cause", "ssp_scenario", "measure", "draw","run_date"],
    task_args=[],
    op_args=[],
)
...
for cause in causes:
    for ssp_scenario in ssp_scenarios:
        for measure in full_measure_map:
            for draw in draws:
                if cause == "malaria":
