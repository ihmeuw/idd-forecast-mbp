...
task_template = tool.get_task_template(
    ...
    node_args=[ "hierarchy", "block_key", "covariate"],  #
    task_args=[], # Only variation is task-specific
    op_args=[],
)


# Compute intersecting block_keys once per hierarchy — same for every covariate,
# so don't recompute inside the covariate loop. Lossless skip of blocks whose
...
for covariate in COVARIATE_DICT.keys():
    ...
    for hierarchy in hierarchies:
        ...
        print(f"Creating tasks for covariate: {covariate}, hierarchy: {hierarchy} ({len(filtered_block_keys)}/{len(block_keys)} blocks)")
