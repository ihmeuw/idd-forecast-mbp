...
# run_date = "2025_07_08"  # Example run date
# delete_existing = True  # Flag to indicate if existing upload folder should be deleted
...
if cause == "malaria":
    processed_forecast_ds_path_template = "{UPLOAD_DATA_PATH}/full_as_malaria_measure_{measure}_ssp_scenario_{ssp_scenario}_dah_scenario_{dah_scenario}_draw_{draw}_with_predictions.nc"
    as_upload_folder_path = f"{UPLOAD_DATA_PATH}/upload_folders/{run_date}/as_cause_{cause}_measure_{measure}_metric_{metric}_ssp_scenario_{ssp_scenario}_dah_scenario_{dah_scenario}"
    aa_upload_folder_path = f"{UPLOAD_DATA_PATH}/upload_folders/{run_date}/aa_cause_{cause}_measure_{measure}_metric_{metric}_ssp_scenario_{ssp_scenario}_dah_scenario_{dah_scenario}"
    
else:
    processed_forecast_ds_path_template = "{UPLOAD_DATA_PATH}/full_as_dengue_measure_{measure}_ssp_scenario_{ssp_scenario}_draw_{draw}_with_predictions.nc"
    as_upload_folder_path = f"{UPLOAD_DATA_PATH}/upload_folders/{run_date}/as_cause_{cause}_measure_{measure}_metric_{metric}_ssp_scenario_{ssp_scenario}"
    aa_upload_folder_path = f"{UPLOAD_DATA_PATH}/upload_folders/{run_date}/aa_cause_{cause}_measure_{measure}_metric_{metric}_ssp_scenario_{ssp_scenario}"
...
# Delete any contents of as_upload_folder_path and aa_upload_folder_path if they exist
for upload_folder_path in [as_upload_folder_path, aa_upload_folder_path]:
...
def get_file_path(draw, cause, measure, ssp_scenario, dah_scenario=None):
...
file_paths = [get_file_path(draw, cause, measure, ssp_scenario, dah_scenario) 
...
upload_ds = xr.open_mfdataset(
    file_paths,
    combine='nested',
    concat_dim='draw_dim',  # This creates a new dimension for the draws
...
upload_ds = upload_ds.assign_coords(draw_dim=ssp_draws)

# Convert to dataset with draw dimension as separate variables
upload_ds = upload_ds['count_pred'].to_dataset(dim='draw_dim')
...
upload_ds = upload_ds_complete

################ Calculate all-age mean and sum ################
aa_ds = upload_ds.sum(dim=['sex_id', 'age_group_id'])
aa_mean_ds = aa_ds.to_array().mean(dim='variable').to_dataset(name='mean_value')
