...
from idd_forecast_mbp.lib.utils.diagnostics import check_column_for_problematic_values
...
def main(
    ...
    den_raked_aa_write_path = Path(den_raked_aa_write_path)

    # Write paths
    mal_raked_aa_write_path.mkdir(parents=True, exist_ok=True)
    den_raked_aa_write_path.mkdir(parents=True, exist_ok=True)
    aa_full_malaria_df_path = mal_raked_aa_write_path / "aa_full_malaria_df.parquet"
    aa_full_dengue_df_path  = den_raked_aa_write_path / "aa_full_dengue_df.parquet"
    aa_full_malaria_ds_path = mal_raked_aa_write_path / "aa_full_malaria_ds.nc"
    aa_full_dengue_ds_path  = den_raked_aa_write_path / "aa_full_dengue_ds.nc"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Rake age-aggregated A2 counts to GBD")
