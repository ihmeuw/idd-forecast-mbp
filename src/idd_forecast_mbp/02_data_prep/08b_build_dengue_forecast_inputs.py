"""Build dengue forecast input netCDFs (one per SSP scenario).

Dengue analog of 08a_build_malaria_forecast_inputs.py. Differences from malaria:
  - NO DAH dimension (dengue has no DAH scenario) -> dims are (location_id, year_id, draw).
  - NO climate_mean / suitability-variant handling; dengue_suitability is a plain
    draw-varying climate covariate (kind=climate_draw), read from the same
    climate-aggregates layout as relative_humidity.
  - Only the REQUESTED climate_draw covariates are read (via wide_to_array on the
    registry-resolved path), not the full 8-var standard set — dengue needs just
    dengue_suitability + relative_humidity.

DEPENDENCY ORDER:
  1. Stage 03 dengue models (R) define the covariate set the --covariates flag matches.
  2. 07c_build_dengue_prediction_locations.py must be run first — this script reads its
     kept-location list and assumes those locs have non-NaN covariates in the forecast
     window (asserted at the end).

Output: one netCDF per requested SSP at
    {DEN_FORECAST_INPUTS_WRITE_PATH}/dengue_forecast_inputs_{ssp}.nc
with dims (location_id, year_id, draw) and the requested covariate variables. Years
span 2000-2100 so predict can both anchor-to-observed on past years and forecast on
future years from a single input file. Past NaN is tolerated (07c only guarantees
forecast-window coverage). Transforms (log, logit) are NOT applied here — raw values on
disk, derived at predict time. Matches the malaria + past-inputs convention.
"""
from __future__ import annotations

import argparse
import collections
from pathlib import Path

import numpy as np
import xarray as xr

from idd_forecast_mbp import constants as mbpc
from idd_forecast_mbp.lib.io.array_builders import (
    read_shared_covariates, scalar_to_array, wide_to_array,
)
from idd_forecast_mbp.lib.io.covariate_registry import (
    COVARIATE_REGISTRY, DEFAULT_DENGUE_FORECAST_COVARIATES, DIMS_BY_KIND,
)
from idd_forecast_mbp.lib.io.netcdf import write_netcdf
from idd_forecast_mbp.lib.io.parquet import read_parquet_with_integer_ids
from idd_forecast_mbp.lib.versioning import finalize_artifact


def _resolve_flooding_path(lsae_hierarchy: str, ssp_scenario: str) -> str | None:
    """Probe the two known flooding filenames; return the first that exists."""
    base = Path(f"/mnt/team/rapidresponse/pub/flooding/results/output/{lsae_hierarchy}/{mbpc.FLOODING_RUN_DATE}")
    for fname in [
        f"fldfrc_weightedmin_sum_{ssp_scenario}_mean_r1i1p1f1.parquet",
        f"fldfrc_shifted0.1_sum_{ssp_scenario}_mean_r1i1p1f1.parquet",
    ]:
        candidate = base / fname
        if candidate.exists():
            return str(candidate)
    return None


def _climate_draw_path(name: str, lsae_hierarchy: str, ssp_scenario: str) -> Path:
    """Registry-resolved path to a draw-varying climate covariate's wide parquet
    (CLIMATE_AGGREGATES_PATH / {lsae} / {name}_{ssp}.parquet)."""
    spec = COVARIATE_REGISTRY[name]
    base = getattr(mbpc, spec.source_path_attr) / lsae_hierarchy
    return base / spec.source_filename.format(ssp_scenario=ssp_scenario)


def main(
    ssp_scenarios: list[str] | None = None,
    covariates: list[str] | None = None,
    lsae_hierarchy: str = mbpc.LSAE_HIERARCHY,
    years: list[int] | None = None,
    hierarchy_read_path: Path = mbpc.HIERARCHY_READ_PATH,
    gdppc_read_path: Path = mbpc.GDPPC_READ_PATH,
    ldipc_read_path: Path = mbpc.LDIPC_READ_PATH,
    urban_read_path: Path = mbpc.URBAN_READ_PATH,
    med_consumppc_read_path: Path = mbpc.MED_CONSUMPPC_READ_PATH,
    forecast_locations_read_path: Path = mbpc.DEN_FORECAST_LOCATIONS_READ_PATH,
    output_path: Path = mbpc.DEN_FORECAST_INPUTS_WRITE_PATH,
) -> None:
    ssp_scenarios = ssp_scenarios or list(mbpc.ssp_scenarios.keys())
    covariates    = list(covariates or DEFAULT_DENGUE_FORECAST_COVARIATES)
    years         = list(years or mbpc.ALL_YEARS)
    output_path   = Path(output_path)
    output_path.mkdir(parents=True, exist_ok=True)

    unknown = set(covariates) - set(COVARIATE_REGISTRY)
    if unknown:
        raise ValueError(
            f"Unknown covariate(s): {sorted(unknown)}. Add to COVARIATE_REGISTRY before requesting."
        )
    bad_kind = {c for c in covariates
                if COVARIATE_REGISTRY[c].kind in ("dah", "climate_mean", "suitability")}
    if bad_kind:
        raise ValueError(
            f"Covariate kind(s) not supported by the dengue builder: {sorted(bad_kind)} "
            f"(dengue has no DAH; suitability is a climate_draw var here)."
        )

    # ── 1. Hierarchy + prediction locations (from 07c) ───────────────────────
    print("Loading hierarchy...")
    hierarchy_df = read_parquet_with_integer_ids(
        Path(hierarchy_read_path) / f"full_hierarchy_2023_{lsae_hierarchy}.parquet"
    )

    print("Loading prediction-location set from 07c artifact...")
    loc_df = read_parquet_with_integer_ids(
        Path(forecast_locations_read_path) / "prediction_location_ids.parquet"
    )
    location_ids = sorted(loc_df["location_id"].astype(int).tolist())
    print(f"  Prediction locations: {len(location_ids)}, years: {len(years)}")

    # ── 2. Static A0 lookup ──────────────────────────────────────────────────
    a0_lookup = (
        hierarchy_df.set_index("location_id")
        .loc[location_ids, "A0_location_id"]
        .values.astype(np.int32)
    )

    # ── 3. Group requested covariates by kind for dispatch ───────────────────
    by_kind: dict[str, list[str]] = collections.defaultdict(list)
    for cov in covariates:
        by_kind[COVARIATE_REGISTRY[cov].kind].append(cov)

    # ── 4. Per-SSP loop: read covariates, assemble Dataset, write ────────────
    for ssp_scenario in ssp_scenarios:
        print(f"\n=== Building forecast inputs for {ssp_scenario} ===")
        rcp_scenario = mbpc.ssp_scenarios[ssp_scenario]["rcp_scenario"]

        data_vars: dict[str, tuple] = {}

        # 4a. Climate (draw-varying) — read ONLY the requested vars, one wide parquet each.
        for name in by_kind["climate_draw"]:
            print(f"Reading {name} (draw-varying)...")
            data_vars[name] = (
                DIMS_BY_KIND["climate_draw"],
                wide_to_array(str(_climate_draw_path(name, lsae_hierarchy, ssp_scenario)),
                              location_ids, years),
            )

        # 4b. Shared scalars + flooding (non-draw)
        if by_kind["shared_scalar"] or by_kind["flooding"]:
            print("Reading shared scalars and flooding...")
            flooding_path = _resolve_flooding_path(lsae_hierarchy, ssp_scenario)
            shared_arrays = read_shared_covariates(
                location_ids=location_ids,
                years=years,
                gdppc_read_path=gdppc_read_path,
                ldipc_read_path=ldipc_read_path,
                urban_read_path=urban_read_path,
                flooding_path=flooding_path,
                rcp_scenario=rcp_scenario,
                med_consumppc_read_path=med_consumppc_read_path,
                variables=by_kind["shared_scalar"] + by_kind["flooding"],
            )
            for name in by_kind["shared_scalar"] + by_kind["flooding"]:
                if name not in shared_arrays:
                    raise KeyError(
                        f"Shared covariate {name!r} not returned by read_shared_covariates. "
                        "Either the source parquet's columns don't match the registry name, "
                        "or read_shared_covariates needs the relevant source path argument."
                    )
                kind = COVARIATE_REGISTRY[name].kind
                data_vars[name] = (DIMS_BY_KIND[kind], shared_arrays[name])

        # 4c. Static lookups
        for name in by_kind["static_lookup"]:
            if name == "A0_location_id":
                data_vars[name] = (DIMS_BY_KIND["static_lookup"], a0_lookup)
            else:
                raise NotImplementedError(
                    f"Static lookup {name!r} has no builder; only A0_location_id supported."
                )

        # 4d. Assemble Dataset + write (dims: location_id, year_id, draw — NO dah_scenario)
        ds = xr.Dataset(
            data_vars=data_vars,
            coords={
                "location_id": np.asarray(location_ids, dtype=np.int32),
                "year_id":     np.asarray(years,        dtype=np.int16),
                "draw":        np.arange(len(mbpc.draws), dtype=np.int16),
            },
        )

        out_file = output_path / f"dengue_forecast_inputs_{ssp_scenario}.nc"
        print(f"Writing {out_file}...")
        write_netcdf(ds, out_file)
        size_gb = out_file.stat().st_size / 1e9
        print(f"  Done. {size_gb:.2f} GB.")

        _assert_no_forecast_window_nan(ds, ssp_scenario)

    if Path(output_path) == Path(mbpc.DEN_FORECAST_INPUTS_WRITE_PATH):
        finalize_artifact(mbpc._A04_DEN_FORECAST_INPUTS)
    else:
        print(f"output_path overridden to {output_path}; skipping finalize_artifact.")


def _assert_no_forecast_window_nan(ds: xr.Dataset, ssp_scenario: str) -> None:
    """Raise if any float data_var has NaN in the forecast window (mbpc.FORECAST_YEARS).

    07c is supposed to drop locations whose covariates are NaN in the forecast window.
    NaN here means 07c wasn't re-run after a source update, or a covariate was added
    without a coverage check.
    """
    future = ds.sel(year_id=slice(mbpc.FORECAST_YEARS[0], mbpc.FORECAST_YEARS[-1]))
    offenders = {}
    for name, da in future.data_vars.items():
        if da.dtype.kind != "f":
            continue  # int / static lookups
        n_nan = int(np.isnan(da.values).sum())
        if n_nan > 0:
            offenders[name] = n_nan
    if offenders:
        raise RuntimeError(
            f"[{ssp_scenario}] forecast-window NaN check failed: {offenders}\n"
            f"Re-run 07c_build_dengue_prediction_locations.py to refresh the "
            f"prediction-location set."
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build dengue forecast input netCDFs")
    parser.add_argument("--ssp_scenarios", nargs="+", default=None,
                        help="SSP scenarios to build. Defaults to all three.")
    parser.add_argument("--covariates", nargs="+", default=None,
                        help="Covariate names to include. Must appear in "
                             "lib/io/covariate_registry.COVARIATE_REGISTRY. "
                             "Defaults to DEFAULT_DENGUE_FORECAST_COVARIATES.")
    parser.add_argument("--lsae_hierarchy", default=mbpc.LSAE_HIERARCHY)
    args = parser.parse_args()
    main(
        ssp_scenarios=args.ssp_scenarios,
        covariates=args.covariates,
        lsae_hierarchy=args.lsae_hierarchy,
    )
