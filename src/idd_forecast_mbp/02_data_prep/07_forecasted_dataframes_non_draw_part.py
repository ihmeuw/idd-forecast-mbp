...
def main(
    ...
    income_paths = {
        "gdppc": str(Path(gdppc_read_path) / "gdppc_mean.parquet"),
    }

    dah_df_path = Path(dah_read_path) / "dah_df.parquet"

    urban_paths = {
        "urban_threshold_300":  str(Path(urban_read_path) / "urban_threshold_300.0_simple_mean.parquet"),
        "urban_threshold_1500": str(Path(urban_read_path) / "urban_threshold_1500.0_simple_mean.parquet"),
    }
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Build non-draw forecasted dataframes")
