...
def main(
    ...
    den_modeling_write_path: Path = mbpc.DEN_MODELING_WRITE_PATH,
    ...
    ###----------------------------------------------------------###
    den_modeling_write_path = Path(den_modeling_write_path)
    den_modeling_write_path.mkdir(parents=True, exist_ok=True)
    aa_ge3_dengue_stage_1_modeling_df_path = den_modeling_write_path / f"aa_ge3_{cause}_stage_1_modeling_df.parquet"
    as_md_dengue_modeling_df_path          = den_modeling_write_path / f"as_md_{cause}_modeling_df.parquet"
    base_md_modeling_df_path               = den_modeling_write_path / f"base_md_{cause}_modeling_df.parquet"
    rest_md_modeling_df_path               = den_modeling_write_path / f"rest_md_{cause}_modeling_df.parquet"

    aa_full_cause_df_path_template = str(Path(den_raked_aa_read_path)) + f'/aa_full_{cause}_df.parquet'
    as_full_cause_df_path_template = str(Path(den_raked_as_read_path)) + f'/as_full_{cause}_df.parquet'
