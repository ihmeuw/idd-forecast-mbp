...
def main(
    ...
    mal_modeling_write_path: Path = mbpc.MAL_MODELING_WRITE_PATH,
    ...
    ###----------------------------------------------------------###
    mal_modeling_write_path = Path(mal_modeling_write_path)
    mal_modeling_write_path.mkdir(parents=True, exist_ok=True)
    aa_ge3_malaria_stage_1_modeling_df_path = mal_modeling_write_path / f"aa_ge3_{cause}_stage_1_modeling_df.parquet"
    aa_md_malaria_pfpr_modeling_df_path     = mal_modeling_write_path / f"aa_md_{cause}_pfpr_modeling_df.parquet"
    as_md_modeling_df_path                  = mal_modeling_write_path / f"as_md_{cause}_modeling_df.parquet"
    base_md_modeling_df_path                = mal_modeling_write_path / f"base_md_{cause}_modeling_df.parquet"
    rest_md_modeling_df_path                = mal_modeling_write_path / f"rest_md_{cause}_modeling_df.parquet"

    aa_full_cause_df_path_template = str(Path(mal_raked_aa_read_path)) + f'/aa_full_{cause}_df.parquet'
    as_full_cause_df_path_template = str(Path(mal_raked_as_read_path)) + f'/as_full_{cause}_df.parquet'
    ...
    # LSAE covariate path (external input, not versioned by this pipeline)
