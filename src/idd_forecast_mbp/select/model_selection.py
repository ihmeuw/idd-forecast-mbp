...
Multi-criteria model-ranking pipeline for malaria scam fits.

Operates on a `metric_df` with one row per fitted model. Columns include:
  - task_id (int)                                      — model identifier
  - is_{aic, bic, loglik, deviance, dev_expl, r_sq, rmse, mae}
  - is_no_fe_{aic, bic, dev_expl, r_sq, rmse, mae}     — comparison only, not ranked
  - is_pfpr_{rmse, mae, r}, is_no_fe_pfpr_{rmse, mae, r}
  - oos_{rmse, mae, r_sq}
  - oos_pfpr_{rmse, mae, r}
...
Ported from idd-tc-mortality/src/idd_tc_mortality/select/model_selection.py.
"""
...
# Placeholder: task_id is unique per model. Extend with structural spec columns
# (e.g., DAH_smooth, A0_af_parametric) merged in from the grid spec before
# cluster_configurations and winner_profile produce useful output.
CONFIG_COLS: List[str] = ["task_id"]
...
def kendall_tau_heatmap(
    ...
    if plot:                    # ← wrap the plotting block
        fig, ax = plt.subplots(figsize=figsize)
...
def cluster_configurations(
    ...
    ----------
    summary_metrics:
        Metric columns to summarise per cluster (mean). Defaults to
        DEFAULT_SUMMARY_METRICS. Missing columns are silently skipped.
    ...
    summary_metrics = summary_metrics if summary_metrics is not None else DEFAULT_SUMMARY_METRICS
...
def run_full_pipeline(
    ...
    ----------
    metric_df:
        One row per fitted model. Columns include metric values and any
        structural config columns referenced by CONFIG_COLS.
    metrics:
    ...
    summary_metrics:
        Metric columns summarised per cluster in cluster_configurations.
    ...
    clustered_df, cluster_summary = cluster_configurations(
        topsis_df, n_clusters=n_clusters, summary_metrics=summary_metrics,
    )
