...
forecast_by_draw_df = read_parquet_with_integer_ids(f"{FORECASTING_DATA_PATH}/malaria_forecast_scenario_{ssp_scenario}_non_draw_part.parquet")
...
covariates_to_log_transform = [
    ...
    # "mal_DAH_intervention_per_capita",
    # "mal_DAH_health_systems_per_capita",
    # "mal_DAH_other_per_capita",
    # "mal_DAH_not_hs_per_capita",
    "gdppc_mean",
    # "ldipc_mean"
]
...
malaria_stage_2_modeling_df = read_parquet_with_integer_ids(malaria_stage_2_modeling_df_path)
# Get the unique values of A0_location_id
columns_to_keep = [
    'location_id',
    'year_id',
    # Add PF columns dynamically
    *[col for col in malaria_stage_2_modeling_df.columns if 'pf' in col],
]
malaria_stage_2_modeling_df = malaria_stage_2_modeling_df[columns_to_keep]
# Make a "stage_2" column that is all 1s
malaria_stage_2_modeling_df["stage_2"] = 1

# Merge in the malaria_stage_2_modeling_df
forecast_by_draw_df = forecast_by_draw_df.merge(
    malaria_stage_2_modeling_df,
    how="left",
...
dah_scenarios, dah_scenario_names = generate_dah_scenarios(
    ...
    malaria_stage_2_modeling_df_path=malaria_stage_2_modeling_df_path
)
...
for dah_scenario_df, dah_scenario_name in zip(dah_scenarios, dah_scenario_names):
    ...
    dah_scenario_df.to_parquet(
        f"{FORECASTING_DATA_PATH}/malaria_forecast_ssp_scenario_{ssp_scenario}_dah_scenario_{dah_scenario_name}_draw_{draw}.parquet",
        index=False
    )

# Write the malaria_stage_2_modeling_df to a parquet file
forecast_by_draw_df.to_parquet(f"{FORECASTING_DATA_PATH}/malaria_forecast_scenario_{ssp_scenario}_draw_{draw}.parquet", compression="snappy", index=False)