...
aa_ge3_dengue_modeling_df_path = glue("{MODELING_DATA_PATH}/aa_ge3_dengue_stage_1_modeling_df.parquet")
as_md_dengue_modeling_df_path = glue("{MODELING_DATA_PATH}/as_md_dengue_modeling_df.parquet")

# Read in a parquet file
aa_dengue_df <- as.data.frame(arrow::read_parquet(aa_ge3_dengue_modeling_df_path))
aa_dengue_df = aa_dengue_df[which(aa_dengue_df$level == 5),]
aa_dengue_df = aa_dengue_df[which(aa_dengue_df$yn == 1),]
aa_dengue_df$A0_af <- as.factor(aa_dengue_df$A0_location_id)
aa_dengue_df$log_dengue_inc_rate <- log(aa_dengue_df$aa_dengue_inc_rate + 1e-6)

tmp_df <- aa_dengue_df[which(aa_dengue_df$aa_dengue_inc_count > 1),]

mod_inc_aa <- scam(log_dengue_inc_rate ~ s(dengue_suitability, k = 6, bs = 'mpi') + total_precipitation + people_flood_days_per_capita,
                  data = tmp_df)
summary(mod_inc_aa)
coef_table <- summary(mod_inc_aa)$coefficients
coef_table[!grepl("as_", rownames(coef_table)) & !grepl("A0_", rownames(coef_table)), ]
summary(mod_inc_aa)$r.squared
...
as_dengue_df <- as_dengue_df[-which(as_dengue_df$age_group_id == 2),]
...
as_dengue_df$as_id <- paste0("a", as_dengue_df$age_group_id, "_s", as_dengue_df$sex_id)
...
# 
base_as_id = "a3_s1"
base_dengue_df = as_dengue_df[as_dengue_df$as_id == base_as_id,]
rest_dengue_df = as_dengue_df[as_dengue_df$as_id != base_as_id,]

tmp_df <- base_dengue_df
tmp_df$base_log_dengue_inc_rate <- tmp_df$log_dengue_inc_rate
tmp_df$base_logit_dengue_cfr <- tmp_df$logit_dengue_cfr



rest_dengue_df = merge(rest_dengue_df, tmp_df[,c("location_id", "year_id", "base_log_dengue_inc_rate", "base_logit_dengue_cfr")],
...
mod_inc_base <- scam(log_dengue_inc_rate ~  s(dengue_suitability, k = 6, bs = 'mpi') + total_precipitation + people_flood_days_per_capita,
                        data = base_dengue_df)
...
mod_inc_rest <- lm(log_dengue_inc_rate ~ base_log_dengue_inc_rate + log_gdppc_mean + as_id,
                   data = rest_dengue_df)




mod_cfr_base <- lm(logit_dengue_cfr ~ log_gdppc_mean + logit_urban_1km_threshold_300 + A0_af,
                   data = base_dengue_df)
...
base_logit_dengue_cfr


mod_cfr_full <- lm(logit_dengue_cfr ~ log_gdppc_mean + logit_urban_1km_threshold_300 + as_id + A0_af,
                   data = as_dengue_df)




summary(mod_cfr_full)

save.image(file = glue("{MODELING_DATA_PATH}/newer_necessary_dengue_regression_models.RData"))
#
...
# 
# mod_inc_all <- lm(log_dengue_inc_rate ~ dengue_suitability + total_precipitation + people_flood_days_per_capita + as_id + A0_af, data = as_dengue_df)
# 
# library(parallel)
# mod_inc_all <- scam(log_dengue_inc_rate ~ s(dengue_suitability, k = 6, bs = 'mpi') + total_precipitation + people_flood_days_per_capita + as_id,data = as_dengue_df,
...
# 
# 
# save.image(file = glue("{data_path}/new_necessary_dengue_regression_models.RData"))
