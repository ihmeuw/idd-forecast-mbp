...
def main(
    ...
    cov_means_write_path: Path = mbpc.COV_MEANS_WRITE_PATH,
    forecasting_data_path: Path = mbpc.FORECASTING_DATA_PATH,
    ...
    hierarchy = lsae_hierarchy
    FORECASTING_DATA_PATH = Path(forecasting_data_path)
    FORECASTING_DATA_PATH.mkdir(parents=True, exist_ok=True)
    VARIABLE_DATA_PATH = str(lsae_input_path)

    cov_means_write_path = Path(cov_means_write_path)
    cov_means_write_path.mkdir(parents=True, exist_ok=True)
    covariate_ds_path = cov_means_write_path / "covariate_means.nc"

    # Hierarchy path
    hierarchy_df_path = Path(hierarchy_read_path) / f"full_hierarchy_2023_{hierarchy}.parquet"
    ...
    flooding_path_template = f"/mnt/team/rapidresponse/pub/flooding/results/output/{hierarchy}/fldfrc_shifted0.1_sum_{{ssp_scenario}}_mean_r1i1p1f1.parquet"
